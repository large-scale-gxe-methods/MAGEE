...
Note: The main effect variance component test (MV) in \textit{MAGEE} is the same as SKAT for related samples \cite{Chen2013}. The main effect hybrid test using Fisher's method test (MF) in \textit{MAGEE} is the same as the efficient hybrid test \textit{SMMAT-E} \cite{Chen2019} in the \href{https://cran.r-project.org/package=GMMAT}{GMMAT} package.  

\section{Getting started}
\subsection{Downloading \textit{MAGEE}}
\textit{MAGEE} is an open source project and is freely available for download at \url{https://github.com/large-scale-gxe-methods/MAGEE}. It can also be found as a regular R package and downloaded from CRAN (\url{https://CRAN.R-project.org/package=MAGEE}).

\subsection{Installing \textit{MAGEE}}
\textit{MAGEE} links to R packages \href{https://cran.r-project.org/package=Rcpp}{Rcpp} and \href{https://cran.r-project.org/package=RcppArmadillo}{RcppArmadillo}, and also imports R packages \href{https://cran.r-project.org/package=Rcpp}{Rcpp}, \href{https://cran.r-project.org/package=CompQuadForm}{CompQuadForm}, \href{https://cran.r-project.org/package=foreach}{foreach}, \href{https://cran.r-project.org/view=HighPerformanceComputing}{parallel}, \href{https://cran.r-project.org/package=Matrix}{Matrix}, methods, \href{https://cran.r-project.org/package=GMMAT}{GMMAT}, \href{https://cran.r-project.org/package=data.table}{data.table}. \textit{MAGEE} requires Bioconductor packages \href{https://bioconductor.org/packages/release/bioc/html/SeqArray.html}{SeqArray} and \href{https://bioconductor.org/packages/release/bioc/html/SeqVarTools.html}{SeqVarTools} to work with genotype files in the GDS format. In addition, \textit{MAGEE} requires \href{https://cran.r-project.org/package=testthat}{testthat} to run code checks during development, and \href{https://cran.r-project.org/package=doMC}{doMC} to run parallel computing in \textbf{glmm.gei} and \textbf{MAGEE} for genotype files in the GDS format (however, \href{https://cran.r-project.org/package=doMC}{doMC} is not available on Windows and these functions will switch to a single thread). These dependencies should be installed before installing \textit{MAGEE}.

For optimal computational performance, it is recommended to use an R version configured with the Intel Math Kernel Library (or other fast BLAS/LAPACK libraries). See the instructions on building R with Intel MKL (\url{https://www.intel.com/content/www/us/en/developer/articles/technical/using-onemkl-with-r.html}).
...
\section{Input}
MAGEE requires an object from fitting the null model using the \textbf{glmmkin} function from the \href{https://cran.r-project.org/package=GMMAT}{GMMAT} package, and a genotype file in a GDS or \href{https://www.well.ox.ac.uk/~gav/bgen_format/spec/latest.html}{BGEN format}. For rare variant analysis, a user-defined group definition file is also required. Specified formats of these files are described as follows. 

\subsection{Object}
MAGEE can perform analysis of gene by multiple environmental factors on multiple traits. To fit the null model, the phenotype and covariates (include the environmental factors of interest) should be saved in a data frame. If the samples are related, the relatedness should be known positive semidefinite matrices $\mb{V}_k$ as an R matrix (in the case of a single matrix) or an R list (in the case of multiple matrices). Refer to the GMMAT user manual (\url{https://cran.r-project.org/package=GMMAT/vignettes/GMMAT.pdf}) to learn the method of fitting the null model. The class of the object should be either "glmmkin" or "glmmkin.multi". 

\subsection{Genotypes}
\textit{MAGEE} can take genotype files either in the GDS format or in any version of the \href{https://www.well.ox.ac.uk/~gav/bgen_format/spec/latest.html}{BGEN format}. Genotypes in Variant Call Format (VCF) and \href{https://www.cog-genomics.org/plink/1.9/formats#bed}{PLINK binary PED format} can be converted to the GDS format using seqVCF2GDS and seqBED2GDS functions from the \href{https://bioconductor.org/packages/release/bioc/html/SeqArray.html}{SeqArray} package:
...
Both \textbf{MAGEE} and \textbf{glmm.gei} requires a "glmmkin" or "glmmkin.multi" class object that contains a fitted GLMM null model. The object can be obtained from the \textbf{glmmkin} function from the R package \href{https://cran.r-project.org/package=GMMAT}{GMMAT}. For more examples and details about the \textbf{glmmkin} function, see the \href{https://cran.r-project.org/package=GMMAT}{GMMAT} manual (\url{https://cran.r-project.org/package=GMMAT/vignettes/GMMAT.pdf}). Below is an example of fitting a GLMM using the \textbf{glmmkin} function from \href{https://cran.r-project.org/package=GMMAT}{GMMAT}:
...
For Mac OS users using R configured with OpenBLAS, the R package \href{https://cran.r-project.org/package=RhpcBLASctl}{RhpcBLASctl} may help set the number of threads used by OpenBLAS to 1. The following lines of code can be used at the beginning of your R script:
...
\subsection{Version 1.4.5 (February 11, 2026)}
\begin{enumerate}
\item Fixed a minor bug in fitglmm.cpp to avoid conflicts between "arma::span" and "std::span" in C++20.
\end{enumerate}
